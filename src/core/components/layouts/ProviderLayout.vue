    ...
    <aside :class="['sidebar-glass', { 'sidebar-open': isMobileMenuOpen }]">
      <div class="brand-section">
        ...
        <div class="brand-text">
          <h1>FuelTrack</h1>
          <span class="badge">Portal Proveedor</span>
        </div>
      ...
      <nav class="nav-menu">
        <router-link to="/provider/dashboard" class="nav-item" active-class="active">
          <LayoutDashboardIcon :size="20" class="nav-icon" />
          <span>Monitor Operativo</span>
        </router-link>

        <router-link to="/provider/orders" class="nav-item" active-class="active">
          <ClipboardListIcon :size="20" class="nav-icon" />
          <span>Gestión de Pedidos</span>
        </router-link>

        <router-link to="/provider/inventory" class="nav-item" active-class="active">
          <DatabaseIcon :size="20" class="nav-icon" />
      ...
      <div class="user-profile-section">
        <div class="user-info">
          ...
          <div class="user-details">
            <span class="company-name">{{ authStore.user?.name || 'Distribuidora de Combustible SAC' }}</span>
            <span class="role-text">Socio Logístico</span>
          </div>
        </div>
      ...
      <header class="topbar">
        <div class="topbar-left">
          ...
          <div class="breadcrumbs">
            ...
            <span class="text-muted">Portal Proveedor</span>
            <ChevronRightIcon :size="14" class="separator" />
...
import { useAuthStore } from '../../../modules/iam/application/stores/authStore';
...
import {
  ...
  LogOutIcon, ChevronRightIcon, BellIcon, MenuIcon, XIcon, DatabaseIcon
...
const currentRouteName = computed(() => {
  if (route.path.includes('dashboard')) return 'Monitor Operativo';
  if (route.path.includes('orders')) return 'Gestión de Pedidos';
  if (route.path.includes('inventory')) return 'Inventario de Planta';
  return 'Centro de Control';
});

onMounted(() => {
  if (notificationStore.sendWelcomeMessage) {
    notificationStore.sendWelcomeMessage('PROVIDER');
  }
});
...
.app-wrapper {
  ...
  width: 100vw;
  display: flex;
  background: radial-gradient(circle at 30% 0%, #0d5446 0%, #03211b 50%, #01120e 100%);
  padding: 24px;
  gap: 0;
...
.nav-item {
  ...
  font-weight: 600;
  font-size: 14px;
  /* 🚀 FIX: Transición muy suave usando cubic-bezier para el botón */
...
/* Preparación de las curvas (existen siempre, pero están ocultas) */
.nav-item::before,
.nav-item::after {
  content: '';
  position: absolute;
  ...
  pointer-events: none;
  z-index: 1;
  opacity: 0; /* 🚀 FIX: Ocultas por defecto */
  transition: opacity 0.4s cubic-bezier(0.4, 0, 0.2, 1); /* Se desvanecen suavemente */
}

/* Forma de esquina cóncava SUPERIOR */
.nav-item::before {
  top: -20px;
...
/* Forma de esquina cóncava INFERIOR */
.nav-item::after {
  bottom: -20px;
  box-shadow: 8px -8px 0 8px #ffffff;
...
.nav-item.active .nav-icon { color: #03211b; }

/* 🚀 FIX: Mostrar las curvas cuando está activo */
...
/* 🚀 FIX: Transición de vistas con cubic-bezier, más suave y elegante */
